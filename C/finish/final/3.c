...
void countBinaryTreeNodeChildren(BinaryTreeNode *root) {
    if (root != NULL) {
        printf("节点 %c 的孩子数目：%d\n", root->data, root->child_count);
        countBinaryTreeNodeChildren(root->left);
        countBinaryTreeNodeChildren(root->right);
    }
}
...
int main() {
    ...
    printf("\n转换后二叉树每个节点的孩子数目：\n");
    countBinaryTreeNodeChildren(binaryTreeRoot);
