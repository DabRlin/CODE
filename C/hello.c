...
int main()
{
    ...
    deleteLinklist(&head, 0); // 删除头节点
    printLinklist(head);

    deleteLinklist(&head, 2); // 删除索引为2的节点
    printLinklist(head);

    deleteLinklist(&head, 1); // 删除索引为1的节点
    printLinklist(head);

    updateLinklist(&head, 100, 0); // 更新索引为0的节点的数据
    printLinklist(head);

    updateLinklist(&head, 200, 1); // 更新索引为1的节点的数据
    printLinklist(head);

    searchLinklist(head, 100); // 查找数据100
    searchLinklist(head, 200); // 查找数据200
    searchLinklist(head, 300); // 查找不存在的数据300

    int data;
    data = getLinklist(head, 0); // 获取索引为0的节点的数据
    if (data != -1)
        printf("Data at index 0 is %d\n", data);

    data = getLinklist(head, 1); // 获取索引为1的节点的数据
    if (data != -1)
        printf("Data at index 1 is %d\n", data);

    data = getLinklist(head, 2); // 获取超出范围的节点
    if (data != -1)
