...
ArcNode *createArcNode(Vertex *head, Vertex *tail);
...
void insertArc(Graph *graph, int headValue, int tailValue)
{
    ...
    ArcNode *newArcNode = createArcNode(headVertex, tailVertex);

    newArcNode->nextSameHead = headVertex->first_out;
    headVertex->first_out = newArcNode;

    newArcNode->nextSameTail = tailVertex->first_in;
    tailVertex->first_in = newArcNode;
